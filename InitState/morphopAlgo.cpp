...
// Return Type  : void
//
void morphopAlgo(const emxArray_real_T *A, emxArray_real_T *B)
{
  ...
  boolean_T exitg2;
  boolean_T exitg1;
  emxInit_real_T2(&out_, 1);
  emxInit_real_T2(&b_B, 1);
  if (!((A->size[0] == 0) || (A->size[1] == 0))) {
    for (i3 = 0; i3 < 2; i3++) {
      np.ImageSize[i3] = A->size[i3];
    }

    for (i3 = 0; i3 < 9; i3++) {
      np.Neighborhood[i3] = true;
    }
    ...
    np.PadValue = 0.0;
    for (i3 = 0; i3 < 2; i3++) {
      k[i3] = np.ImageSize[i3];
    }

    for (i3 = 0; i3 < 9; i3++) {
      nhConn[i3] = np.Neighborhood[i3];
    }
    ...
    if (!(centerLocationInImage == 0.0)) {
      ...
      for (pind = 0; pind < 9; pind++) {
        if (nhConn[pind]) {
          ...
          for (i3 = 0; i3 < 2; i3++) {
            imnhSubs[indx + 9 * i3] = subs[i3];
            firstIndRange[i3] = b_subs[i3] * (1 + (i3 << 1));
          }
          ...
          for (i3 = 0; i3 < 2; i3++) {
            firstIndRange[i3] = c_subs[i3] * pixelsPerImPage[i3];
          }
      ...
      for (i3 = 0; i3 < 2; i3++) {
        firstIndRange[i3] = (2 - i3) * pixelsPerImPage[i3];
      }

      centerLocationInImage = (double)firstIndRange[0] + (double)firstIndRange[1];
      for (i3 = 0; i3 < 9; i3++) {
        nhInds[i3] -= (int)centerLocationInImage;
      }
    ...
    for (i3 = 0; i3 < 2; i3++) {
      np.InteriorStart[i3] = 2.0;
    }

    for (i3 = 0; i3 < 2; i3++) {
      np.InteriorEnd[i3] = k[i3];
    }
    ...
    in->size[0] = A->size[0];
    in->size[1] = A->size[1];
    emxEnsureCapacity((emxArray__common *)in, i3, (int)sizeof(double));
    indx = A->size[0] * A->size[1];
    for (i3 = 0; i3 < indx; i3++) {
      in->data[i3] = A->data[i3];
    }
...
#pragma omp for nowait
      ...
      for (secondInd = ck; secondInd <= ak; secondInd++) {
        ...
        b_i = b_B->size[0];
        b_B->size[0] = b_pind;
        emxEnsureCapacity((emxArray__common *)b_B, b_i, (int)sizeof(double));
        ...
        emxEnsureCapacity((emxArray__common *)out_, b_i, (int)sizeof(double));
        for (b_firstInd = firstIndRange[0]; b_firstInd <= firstIndRange[1];
          ...
          varargin_1_data[imnh_size[0] * imnh_size[1]] = rtMinusInf;
          b_pind = 1;
    ...
    if (nhoodObj->ProcessBorder) {
      ...
      centerLocationInImage = nhoodObj->PadValue;
      ...
      for (b_secondInd = 0; b_secondInd < b_minval; b_secondInd++) {
        for (firstInd = 1; firstInd <= minval; firstInd++) {
          ...
          for (i3 = 0; i3 < 2; i3++) {
            firstIndRange[i3] = nhoodObj->ImageSize[i3];
          }
          ...
          pixelSub[0] = r;
          for (i3 = 0; i3 < 2; i3++) {
            centerPixelSub[i3] = pixelSub[i3];
          }

          for (i3 = 0; i3 < 18; i3++) {
            a[i3] = nhoodObj->NeighborSubscriptOffsets[i3];
          }
          ...
          if (p) {
            for (i3 = 0; i3 < u0; i3++) {
              tmp_data[i3] = imnhInds_data[i3];
            }

            ak = 1;
            ck = u0;
            for (i3 = 0; i3 < u0; i3++) {
              b_imnh_data[i3] = in->data[tmp_data[i3] - 1];
            }
          } else {
            ak = u0;
            ck = 1;
            for (i3 = 0; i3 < u0; i3++) {
              b_imnh_data[i3] = in->data[imnhInds_data[i3] - 1];
            }
          }
          ...
          if (nhoodObj->Padding == 2.0) {
            for (i = 0; i < 9; i++) {
              if (!nhConn[i]) {
                b_imnh_data[i] = centerLocationInImage;
              }
            }
          }
          ...
          indx = ak * ck;
          for (i3 = 0; i3 < indx; i3++) {
            b_varargin_1_data[i3] = b_imnh_data[i3];
          }

          b_varargin_1_data[ak * ck] = rtMinusInf;
          indx = 1;
          b_mtmp = b_varargin_1_data[0];
          if (i > 1) {
            if (rtIsNaN(b_varargin_1_data[0])) {
              ak = 2;
              exitg10 = false;
              while ((!exitg10) && (ak <= i)) {
                indx = ak;
                if (!rtIsNaN(b_varargin_1_data[ak - 1])) {
      ...
      while (u0 <= b_minval) {
        for (firstInd = 1; firstInd <= minval; firstInd++) {
          ...
          b_pixelSub[0] = r;
          for (i3 = 0; i3 < 2; i3++) {
            centerPixelSub[i3] = b_pixelSub[i3];
          }
          ...
          if (p) {
            for (i3 = 0; i3 < trueCount; i3++) {
              tmp_data[i3] = b_imnhInds_data[i3];
            }
            ...
            ck = 1;
            for (i3 = 0; i3 < trueCount; i3++) {
              b_imnh_data[i3] = in->data[b_imnhInds_data[i3] - 1];
            }
          }
      ...
      for (b_secondInd = 1; b_secondInd <= b_minval; b_secondInd++) {
        for (firstInd = 0; firstInd < minval; firstInd++) {
          ...
          c_pixelSub[0] = r + 1;
          for (i3 = 0; i3 < 2; i3++) {
            centerPixelSub[i3] = c_pixelSub[i3];
          }
          ...
          p = (in->size[0] == 1);
          if (p) {
            for (i3 = 0; i3 < b_trueCount; i3++) {
              tmp_data[i3] = c_imnhInds_data[i3];
            }
            ...
            ck = 1;
            for (i3 = 0; i3 < b_trueCount; i3++) {
              b_imnh_data[i3] = in->data[c_imnhInds_data[i3] - 1];
            }
          }
      ...
      for (b_secondInd = 1; b_secondInd <= b_minval; b_secondInd++) {
        for (firstInd = u0; firstInd <= minval; firstInd++) {
          ...
          d_pixelSub[0] = r;
          for (i3 = 0; i3 < 2; i3++) {
            centerPixelSub[i3] = d_pixelSub[i3];
          }
          ...
          p = (in->size[0] == 1);
          if (p) {
            for (i3 = 0; i3 < c_trueCount; i3++) {
              tmp_data[i3] = d_imnhInds_data[i3];
            }
            ...
            ck = 1;
            for (i3 = 0; i3 < c_trueCount; i3++) {
              b_imnh_data[i3] = in->data[d_imnhInds_data[i3] - 1];
            }
          }
